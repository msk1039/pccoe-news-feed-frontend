...
export default function NewsFeed() {
  ...
  const saveDislikedPosts = (dislikedPosts: Set<number>) => {
    localStorage.setItem(
      "dislikedPosts",
      JSON.stringify(Array.from(dislikedPosts))
    );
  };
  ...
  const saveCreatedPosts = (createdPosts: Set<number>) => {
    localStorage.setItem(
      "createdPosts",
      JSON.stringify(Array.from(createdPosts))
    );
  };
  ...
  return (
    <div className="w-full bg-[#f0f0f0] font-[family-name:var(--font-geist-sans)]">
      <div className="container mx-auto py-8 px-4 bg-[#f0f0f0] min-h-screen"> 
        ...
        <p className="text-xl font-semibold text-center mb-8 font-[family-name:var(--font-geist-sans)]">
          Stay up to date with the latest news from PCCOE
        </p>
        {/* // add site description */}
              ...
              <form onSubmit={handleSubmit}>
                ...
                <DialogFooter>
                  <DialogClose asChild>
                    <Button
                      type="submit"
                      className="bg-black text-white hover:bg-white hover:text-black border-2 border-black transition-all duration-300 font-bold"
                    >
                      Save Post
                    </Button>
                  </DialogClose>
          ...
          <ScrollArea className="h-[calc(100vh-200px)]">
            <div className="space-y-4 mr-3">
              {[...news].reverse().map((item) => (
                <Card
                  key={item.id}
                  className="border-4 border-black hover:shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] transition-all duration-300"
                >
                    ...
                    <div className="flex flex-col items-end space-y-2 ml-4">
                      <div className="flex items-center space-x-2">
                        <Button
                          ...
                          className={`border-2 border-black hover:bg-green-300 transition-all duration-300 ${
                            likedPosts.has(item.id) ? "bg-green-300" : ""
                          }`}
                        >
                        ...
                        <Button
                          ...
                          className={`border-2 border-black hover:bg-red-300 transition-all duration-300 ${
                            dislikedPosts.has(item.id) ? "bg-red-300" : ""
                          }`}
                        >
